#' @title Agent Adds Edges by Randomly Selecting Alters
#' @description
...
actAgent_addEdges_random <- function(D,
                                     num_edges = 1,
                                     alter_candidate = "new",
                                     which_net = 1,
                                     value = 1,
  ...
  current_network <- self$e[[which_net]]
  ...
  if(alter_candidate[1] %in% c("new", "known", "all")){
    switch(alter_candidate,
           "new" = {
             candid <- setdiff(
               names(D$agents)[names(D$agents)!=self$a$ID],
               names(current_network))
           },
           "known" = {
  ...
  if(!is.null(.valueFunction)){
    ...
    names(new_alter_value) <- new_alter
  }else{
    # Functionが提供されていない場合
  ...
  # 新しいエッジを足す
  new_edges <- c(self$e[[which_net]], new_alter_value)
  self$e[[which_net]] <- tapply(new_edges, names(new_edges), sum)
}
