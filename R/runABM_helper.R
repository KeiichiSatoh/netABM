...
.shape_selectFUN <- function(selectFUN_sbs = selectFUN_sbs){
  ...
  if(selectFUN_sbs=="all"){
    ## "all"の場合
    selectFUN_shaped <- function(D = D){
      agent_n <- memo_length(D$agent)
      sample(1:agent_n)}
    selectFUN_label <- "all"
    ...
    if(selectFUN_name=="function"){
      ...
      formals(selectFUN_shaped) <- c(alist(D = D), current_args)
...
.shape_updateFUN <- function(updateFUN_sbs = updateFUN_sbs){
  ...
  if(is.null(updateFUN_sbs)){
    ...
    if(updateFUN_name=="function"){
      ...
      current_args[which(names(current_args)=="D")] <- NULL
      formals(updateFUN_shaped) <- c(alist(D = D), current_args)
      ...
      for(i in 1:length(updateFUN_args_override)){
        current_args[which(names(current_args)==names(updateFUN_args_override)[i])] <- repeatFUN_args_override[i]
      }
      current_args[which(names(current_args)=="D")] <- NULL
      formals(updateFUN_shaped) <- c(alist(D = D), current_args)
...
.construct_do_repeat <- function(save_log = save_log){
  # ひな形を作る
  do_repeat <- function(D, stopFUN, stopFUN_args,
