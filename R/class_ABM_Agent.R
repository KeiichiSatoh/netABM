...
#' @description
#' The \code{ABM_Agent} class provides methods and fields for creating and managing
#' agent's action and attributes in an agent-based model (ABM). It allows agents to have custom fields, methods,
#' and active bindings, making this class highly flexible for various simulation scenarios.
#'
#' @details
#' `ABM_Agent` is one of the core classes of `xABM`.
#' `ABM_Agent` is the class for agents that includes:
#' - A initialize method (i.e., `new()`)
#' - Methods for adding/removing fields, methods, and active bindings (i.e., `.add_field()`, `.add_method()`, `.add_active_binding()`, `remove_field()`).
...
#'
#' This class is implemented using the `R6` class. While agents can be instantiated
#' directly using `ABM_Agent$new()`, it is highly recommended to use \link{init_agent}
#' for batch creation of agents for the actual usage.
#' The `init_agent` function provides the navigation for setting various field types
#' and includes validation checks that is not present in the `ABM_Agent` class method.
#'
...
#' Field names are set as attributes, while methods are assigned as callable functions.
...
ABM_Agent <- R6::R6Class("ABM_Agent", lock_objects = FALSE, cloneable = TRUE,
                         public = list(
                           ...
                           #' - act_FUN: Actions of the agenets.
